...
type SideSelectorProps = {
  ...
  matchOtherSide: (func: boolean, trace: boolean) => void;
};
...
function MatchButton({
  ...
  const handleMatch = () => {
    matchOtherSide(checked.func, checked.trace);
  ...
  return (
    ...
    >
      <header className="side-selector__heading">
        ...
        <div>
          <div>
            <MatchButton matchOtherSide={matchOtherSide} />
          </div>
        </div>
      </header>
...
const ThemeSelector = memo(function ThemeSelector({
  ...
  return (
    <div className="content editor-theme-selector">
...
function NavigationPair({
  ...
  return (
    <div className="navigation-pair">
      <span className="navigation-pair__label">{label}</span>

      <div className="navigation-pair__buttons">
        <button
          type="button"
          title={previousLabel}
...
function App() {
  ...
  const activeFunctionName =
    index?.functions.find(
      (identity) => identity.id === activeSelection?.functionId,
    )?.internalName ?? "";
  const inliningTimelineBranches = useMemo<InliningTimelineBranch[]>(() => {
    if (!cache) return [];
    return timeline.flatMap((entry) =>
      entry.terminalInliningDestinations.flatMap((destination) => {
        if (!destination.functionId) return [];
        const trace = cache.snapshotIdToTrace.get(destination.snapshotId);
        if (!trace) return [];
        return [
          {
            id: `${entry.snapshot.id}:${destination.functionId}:${destination.snapshotId}`,
            sourceSnapshotId: entry.snapshot.id,
            destination,
            trace,
            timeline: timelineFor(trace, destination.functionId, cache),
          },
        ];
      }),
    );
  ...
  const matchAfterSide = (func: boolean, trace: boolean) => {
    let nextFuncId = before!.functionId;
    let nextSnapshotId = before!.snapshotId;
    if (func) {
      nextFuncId = after!.functionId;
    }
    if (trace) {
    ...
    updateBefore({
      functionId: nextFuncId,
      snapshotId: nextSnapshotId,
    });
  };
  const matchBeforeSide = (func: boolean, trace: boolean) => {
    let nextFuncId = after!.functionId;
    let nextSnapshotId = after!.snapshotId;
    if (func) {
      nextFuncId = before!.functionId;
    }
    if (trace) {
            ...
            {!sideSelectorsCollapsed && (
              <>
                ...
                <SideSelector
                  ...
                  matchOtherSide={matchBeforeSide}
          ...
          <section className="timeline" aria-label="Function timeline">
            ...
            {!timelineCollapsed && (
              <>
                ...
                {timelineExpanded &&
                activeTrace &&
                inliningTimelineBranches.length > 0 ? (
                  <ExpandedInliningTimeline
